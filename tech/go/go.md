...
1. 协程：标准库所有系统调用（当然包括同步IO）都会让出CPU 给其他协程。运行时调度，非常轻量，可以轻松创建百万个
   1. 可增长的分段堆栈，也就是只在需要时才使用更多内存
   2. 比线程更快的启动速度
   3. 原生支持channel进行通信
   4. 共享数据结构时无需使用互斥锁

先前，go代码文件都被组织成包(package)放在GOPATH下的src进行管理，在1.11 版本引入module 概念后用以替换基于GOPATH 的管理方式（代码可以写在任意位置），支持多版本共存（1.13版本以前Go Module默认关闭，需要设置GO111MODULE=on 打开）

不允许定义变量/导入包而不使用，但可以用`_`，表示丢弃返回（也可以用于函数的多返回值中）
`_`不占命名空间，不分配内存，所以不存在重复声明

静态强类型，自带gc

# 开始
设置环境变量GOROOT 为go 的安装目录（不是go 这个二进制的目录，而是bin 所在的目录）
设置环境变量GOPATH 为工作空间目录（也就是依赖包的下载目录，该环境变量可以是多个目录，默认下载到第一个，而查找包则会按顺序依次查找）
版本查看`go version`
...
## 执行
直接使用`go run src.go` 就可以编译链接并执行该源文件（可执行文件放在临时目录中）
也可以使用`go build src.go` 将该源文件编译为可执行文件（静态编译，不用担心动态库的版本依赖问题），可以使用`-o`选项指定产出的可执行文件名
编译前，通过设置环境变量进行跨平台编译：
```shell
CGO_ENABLED=0	//禁用CGO
GOOS=linux		//目标操作系统，mac是darwin，win是windows
...
变量会在声明时进行初始化
```go
var var_name type1  // 自动初始化为默认值（类型零值，例如0、""、false、nil）
var var_name type2 = val  // 初始化
var (
	v1 type1
	v2 type2
)
var var_name1, var_name2 = val1, val2  // 初始化，用val 进行类型推断
...
该命令会在当前目录下生成一个go.mod 文件

### 安装一个模块
执行`go install [<module-prefix>]`，就会将该模块进行编译成二进制文件，然后放入指定目录：
1. 若GOBIN 环境变量设置，则放入该目录；
2. 若GOPATH 环境变量设置，则放入该目录的bin 下（通过`go env GOPATH` 可以查看该环境变量的默认值）
